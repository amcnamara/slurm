...
(defn -main []
  ...
  (let [slurmdb    (slurm/init db-schema)
	course1    (slurm/.create slurmdb (DBConstruct. :course {:name "Software Design I"}))
	course2    (slurm/.create slurmdb (DBConstruct. :course {:name "Intro to Programming"}))
	cs-degree  (slurm/.create slurmdb (DBConstruct. :degree {:name "Computer Science, B.Sc." :courses [course1, course2]}))]
